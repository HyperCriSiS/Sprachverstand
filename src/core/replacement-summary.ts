...
function trimSharedContext(
  ...
  while (start > 0 && isWordLikeCharacter(original[start - 1] ?? "")) {
    start -= 1;
  }

  while (
    originalEnd < original.length &&
    isWordLikeCharacter(original[originalEnd] ?? "")
  ) {
    originalEnd += 1;
  }
  ...
  while (
    ...
    isWordLikeCharacter(transformed[transformedEnd] ?? "")
  ) {
    transformedEnd += 1;
  }
